...
document.getElementById('ctx-delete').addEventListener('click', () => {
    if (contextTarget && currentServerId) {
        if(confirm(`Delete this ${contextTarget.type}?`)) {
            if(contextTarget.type === 'channel') {
                ...
                remove(ref(db, `categories/${currentServerId}/${contextTarget.id}`));
            }
        }
    }
});
...
document.getElementById('avatar-upload').addEventListener('change', (e) => {
    ...
    if (file) { const reader = new FileReader(); reader.onloadend = () => { tempBase64Avatar = reader.result; document.getElementById('profile-preview').src = tempBase64Avatar; }; reader.readAsDataURL(file); }
});
        ...
        snap.forEach((childSnapshot) => {
            ...
            get(child(ref(db), `servers/${serverId}`)).then((sSnap) => {
                if (sSnap.exists()) {
                    ...
                    const div = document.createElement('div'); div.classList.add('server-icon'); div.id = `server-${serverId}`;
                    ...
                    div.addEventListener('click', async () => {
                        ...
                        if (sData.owner === auth.currentUser.email || roleId === 'owner') { myServerPerms = { admin: true, manageChannels: true, deleteMessages: true }; } 
...
document.getElementById('delete-server-btn').addEventListener('click', async () => {
    if(confirm("Are you ABSOLUTELY sure you want to delete this server? This will wipe all channels, roles, and messages.")) {
        if(currentServerId) {
            await remove(ref(db, `servers/${currentServerId}`));
            ...
            customAlert("Server deleted successfully.");
        }
    }
});
    ...
    onValue(ref(db, `servers/${currentServerId}/roles`), (snap) => {
        ...
        rolesArray.forEach(rData => {
            ...
            div.innerHTML = `
                ...
                <div>
                    <label style="font-size:11px; margin-right:5px;"><input type="checkbox" ${rData.perms.admin?'checked':''} class="r-perm" data-role="${roleId}" data-perm="admin"> Admin</label>
                    ...
                    <label style="font-size:11px;"><input type="checkbox" ${rData.perms.deleteMessages?'checked':''} class="r-perm" data-role="${roleId}" data-perm="deleteMessages"> Del Msg</label>
                </div>`;
...
// Global Sync for Channels AND Categories (fixes category bug)
    ...
    sortedCats.forEach(catId => {
        ...
        if(catId !== "uncategorized") {
            ...
            catDiv.addEventListener('drop', (e) => {
                e.stopPropagation(); catDiv.classList.remove('drag-over');
                if(dragSrcEl) {
                    const srcId = dragSrcEl.id.replace('channel-', '');
                    update(ref(db, `channels/${serverId}/${srcId}`), { categoryId: catId, order: Date.now() });
                }
                return false;
            });
        ...
        grouped[catId].sort((a,b) => (a.order||0) - (b.order||0)).forEach((channelData, index, arr) => {
            ...
            channelList.appendChild(div);
            if (unreadState.channels.has(channelData.id)) updateBadge(`channel-${channelData.id}`, true, false);
        });
    });
}
...
function initVoiceChat() { 
    ...
    myPeer.on('call', call => { call.answer(localAudioStream); const cEmail = call.metadata ? call.metadata.callerEmail : call.peer; call.on('stream', stream => addVoiceUserUI(cEmail, stream)); activeCalls[cEmail] = call; call.on('close', () => removeVoiceUserUI(cEmail)); }); 
}
...
async function buildMessageHtml(data) {
    let contentHtml = `<div style="margin-left: 42px; word-break: break-word; color: #dcddde;">${data.text || ''}</div>`;
    ...
    while ((match = inviteRegex.exec(data.text)) !== null) {
        ...
        if(sSnap.exists()) {
            const sData = sSnap.val();
            const iconHtml = sData.icon ? `<div class="invite-embed-icon" style="background-image:url(${sData.icon})"></div>` : `<div class="invite-embed-icon">${sData.name.charAt(0)}</div>`;
            contentHtml += `
