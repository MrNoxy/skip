...
// Shows a settings tab pane with a small fade+slide entrance instead of the
// instant, jarring display:none -> display:block switch it used to be. Toggling
// the class off then back on (with a forced reflow in between) makes sure the
// animation replays every time a pane is (re)shown, not just the first time.
function showPaneAnimated(el) {
    if (!el) return;
    el.style.display = 'block';
    el.classList.remove('pane-anim-in');
    void el.offsetWidth; // force reflow so re-adding the class below restarts the animation
    el.classList.add('pane-anim-in');
}
...
function setupUserSettingsTabs() {
    document.querySelectorAll('#user-settings-modal .fs-tab').forEach(tab => {
        tab.addEventListener('click', (e) => {
            ...
            showPaneAnimated(document.getElementById(`pane-us-${tabName}`));
...
let studioJustAddedElId = null; // set right before renderStudio() when a new element was just added, so only it pops in

// The canvas is a fixed 340×314 design (element x/y coordinates are stored in that
// space) so it always looks the same everywhere — but 340px is wider than most
// phone screens, which used to force horizontal scrolling to see the rest of the
// Background Studio. Instead we scale the whole canvas down with a CSS transform
// to fit whatever width is actually available, and adjust drag math by the same
// factor so dragging still tracks the finger/cursor 1:1 on screen.
function getStudioScale() {
    return document.getElementById('studio-canvas-outer')?._studioScale || 1;
}
function fitStudioCanvas() {
    const outer = document.getElementById('studio-canvas-outer');
    const canvas = document.getElementById('studio-canvas');
    if (!outer || !canvas || !canvas.offsetParent) return; // hidden (e.g. a different settings tab is showing) — nothing to measure yet
    const row = outer.closest('.studio-row');
    const isColumnLayout = window.innerWidth <= 768; // matches the CSS breakpoint that stacks .studio-row
    const controlsReserve = isColumnLayout ? 0 : 220; // ~min-width of .studio-controls + gap, reserved so the canvas doesn't crowd it out on desktop
    const available = Math.max(160, (row ? row.clientWidth : 340) - controlsReserve);
    const scale = Math.min(1, available / 340);
    canvas.style.transform = scale < 1 ? `scale(${scale})` : '';
    outer.style.width = (340 * scale) + 'px';
    outer.style.height = (314 * scale) + 'px';
    outer._studioScale = scale;
}
window.addEventListener('resize', debounce(fitStudioCanvas, 120));

function renderStudio() {
    fitStudioCanvas();
    // The modal (or this tab) can still be `display:none` at the exact moment
    // renderStudio() is called — e.g. it's called right before the modal itself
    // is shown. Re-measure one frame later as a safety net for that ordering.
    ...
    if(elementsLayer) {
        ...
        (studioState.elements || []).forEach((el, index) => {
            ...
            // ---- Drag to reposition — mouse (desktop) ----
            // NOTE: previously attached fresh document-level mousemove/mouseup
            // listeners on every single renderStudio() call (which happens on
            // every color tweak, angle drag tick, etc.) and never removed them —
            // a listener leak that piled up dozens of dead closures per settings
            // session. These are now added on mousedown and explicitly removed
            // on mouseup, same pattern as the chat sticker engine.
            ...
            function onMove(e) {
                ...
                el.x = origX + (e.clientX - startX) / scale;
                el.y = origY + (e.clientY - startY) / scale;
                domEl.style.left = el.x + 'px';
                domEl.style.top = el.y + 'px';
            }
            function onUp() {
            ...
            // ---- Drag to reposition — touch (mobile) ----
            // Long-press to remove (mirrors the right-click affordance on desktop,
            ...
            domEl.addEventListener('touchmove', (e) => {
                ...
                e.preventDefault();
                const t = e.touches[0];
                if (Math.hypot(t.clientX - startX, t.clientY - startY) > 6) { touchMoved = true; clearTimeout(longPressTimer); }
                const scale = getStudioScale();
                el.x = origX + (t.clientX - startX) / scale;
                el.y = origY + (t.clientY - startY) / scale;
                domEl.style.left = el.x + 'px';
                domEl.style.top = el.y + 'px';
            }, { passive: false });
            domEl.addEventListener('touchend', () => clearTimeout(longPressTimer), { passive: true });

            elementsLayer.appendChild(domEl);
        });
    }
    studioJustAddedElId = null;
}
...
document.getElementById('studio-add-text-btn')?.addEventListener('click', () => {
    openInputModal("Add Text", "Type something...", "", (text) => {
        if(text) {
            const newId = generateCode();
            studioState.elements.push({ id: newId, type: 'text', content: text, x: 50, y: 50 });
            studioJustAddedElId = newId;
            renderStudio();
        }
    });
...
document.getElementById('create-server-btn')?.addEventListener('click', () => {
    openInputModal("Create Server", "Server Name", "Give your server a name:", (serverName) => {
        if (serverName) {
            ...
            const everyonePerms = { viewChannels: true, sendMessages: true, placeEmojiStickers: true, manageChannels: false, manageServerSettings: false, manageServerProfile: false, manageServerOverview: false, manageRoles: false, manageMessages: false, kickMembers: false, banMembers: false, timeoutMembers: false, manageDownloadChannel: false };
...
function editRole(roleId, noSwitch = false) {
    ...
    const p = rData.perms || {};
    ['viewChannels', 'sendMessages', 'placeEmojiStickers', 'manageChannels', 'manageServerSettings', 'manageServerProfile', 'manageServerOverview', 'manageRoles', 'manageMessages', 'kickMembers', 'banMembers', 'timeoutMembers', 'manageDownloadChannel'].forEach(perm => {
...
document.querySelectorAll('#channel-settings-modal .fs-tab').forEach(tab => {
    tab.addEventListener('click', (e) => {
        ...
        showPaneAnimated(document.getElementById(`pane-cs-${tabName}`));
...
async function fetchOlderMessages() {
    if (!oldestMsgTimestamp || isFetchingMore) return;
    isFetchingMore = true;
...
function resolveAnchor(clientX, clientY) {
    ...
    if (anchorEl) {
        ...
        // Also record a coarse fallback position as a fraction of the *entire*
        // scrollable history (not just the visible pane) at placement time. Only
        // the most recent 50 messages are loaded when a chat is opened, so if this
        // anchor message hasn't been paged in yet (or was later deleted),
        // computeStickerPixelPos can reconstruct roughly the right spot from this
        // instead of defaulting to dead-center — which is what used to make a
        // sticker look like it had teleported to a "completely different spot".
        return {
            ...
            fy: (r.top - messagesRect.top + messagesDiv.scrollTop) / Math.max(1, messagesDiv.scrollHeight),
        };
    }
...
function computeStickerPixelPos(s) {
    ...
    if (x === undefined) {
        // Anchor message isn't in the DOM — either it's genuinely gone (deleted),
        // or (much more commonly) it simply hasn't been paged in yet because only
        // the most recent 50 messages load up front. backfillStickerAnchors() tries
        // to page in older history before we ever get here, but as a safety net:
        x = (s.fx != null ? s.fx : 0.5) * messagesRect.width;
        if (s.anchorMsgId && s.fy != null) {
            // fy was captured as a fraction of the *entire* scroll history at
            // placement time, so — unlike the visible-pane fraction below — it
            // still points at roughly the right spot even after pagination.
            y = s.fy * messagesDiv.scrollHeight;
        } else {
...
// Only the most recent 50 messages are loaded when a chat is opened (see
// loadMessages/fetchOlderMessages). A sticker anchored to an older message would
// therefore have nothing to attach to right after a reload — computeStickerPixelPos
// would fall back to an approximate spot instead of the real one. This quietly
// pages in older history (reusing the exact same "load older" path as scrolling
// up, which already preserves scroll position) until every known sticker's anchor
// is actually present, then repositions them precisely. This is the main fix for
// stickers appearing to "teleport" after a reload.
async function backfillStickerAnchors(chatId) {
    const MAX_BATCHES = 12; // ~600 messages of headroom; stops a deleted/bad anchor from looping forever
    for (let i = 0; i < MAX_BATCHES; i++) {
        if (chatId !== currentChatId || !oldestMsgTimestamp) break;
        const stickers = stickersByChatId[chatId] || [];
        const stillUnresolved = stickers.some(s => s.anchorMsgId && !document.getElementById(s.anchorMsgId));
        if (!stillUnresolved) break;
        await fetchOlderMessages();
    }
    if (chatId === currentChatId) repositionAllStickers();
}
...
async function subscribeStickers(chatId) {
    stickersByChatId[chatId] = [];
    updateStickerHideBtn();

    const stickersRef = ref(db, `${STICKERS_DB_ROOT}/${chatId}`);

    // Load the current snapshot up front (rather than only ever discovering
    // stickers reactively via onChildAdded) so we know exactly what's placed
    // before wiring up live listeners — that's what lets backfillStickerAnchors
    // run deterministically once, right after load, instead of racing it.
    try {
        const snap = await get(stickersRef);
        if (chatId !== currentChatId) return; // switched chats while this was loading
        if (snap.exists()) {
            snap.forEach(c => {
    ...
    unsubscribeStickers = onChildAdded(stickersRef, (snap) => {
        ...
        if ((stickersByChatId[chatId] || []).some(x => x.id === snap.key)) return; // already loaded above, or our own optimistic write
        const s = { id: snap.key, ...snap.val() };
        if (!stickersByChatId[chatId]) stickersByChatId[chatId] = [];
        stickersByChatId[chatId].push(s);
        appendStickerEl(s, true); // genuinely new live placement — worth the pop-in
        updateStickerHideBtn();
    });
    ...
    unsubscribeStickersChanged = onChildChanged(stickersRef, (snap) => {
        ...
        // BUG FIX: this used to do `list[idx] = incoming`, which swapped in a brand
        // new object instead of updating the existing one in place. The sticker's
        // DOM element closures (drag/resize/rotate handlers in appendStickerEl) hold
        // a reference to the *original* object, so once the array pointed somewhere
        // else, any later local edit (resize, drag, rotate) kept mutating the now-
        // orphaned original — invisible to future lookups like the `.find()` in
        // repositionAllStickers(). The next reposition (window resize, older-message
        // load, etc.) would then read the stale orphaned copy's size/anchor and the
        // sticker would visibly "snap back" to whatever it was before your edit.
        // Mutating the same object in place keeps every reference consistent.
        if (idx >= 0) Object.assign(list[idx], incoming);
        else list.push(incoming);
        updateStickerElFromData(list[idx] ?? incoming);
    });
...
function applyTransform(el, s) {
    // Exposed as a CSS var too so the pop-in keyframe (which animates its own
    // `transform`) can still respect this sticker's rotation instead of fighting it.
    el.style.setProperty('--s-rot', (s.rot || 0) + 'deg');
    el.style.transform = `translate(-50%, -50%) rotate(${s.rot}deg)`;
}

// Applies fresh data (from another client, or the Firebase echo of our own write)
// to an already-rendered sticker element.
function updateStickerElFromData(s) {
    ...
    applyTransform(el, s);
    // Our own writes echo back through this same listener. If that echo lands
    // while (or just after) the user is actively dragging/resizing/rotating this
    // exact sticker, recomputing its position here can race the gesture and make
    // it visibly "snap" — even though the underlying data is correct. Skip the
    // reposition in that narrow window; the gesture's own end-handler already
    // sets the correct final position once it commits.
    if (el._suppressRepositionUntil && Date.now() < el._suppressRepositionUntil) return;
    const { x, y } = computeStickerPixelPos(s);
    el.style.left = x + 'px';
    el.style.top = y + 'px';
}
...
function appendStickerEl(s, animateIn) {
    const layer = document.getElementById('emoji-sticker-layer');
    if (!layer) return;
    ...
    if (animateIn) {
        el.classList.add('sticker-pop-in');
        el.addEventListener('animationend', () => el.classList.remove('sticker-pop-in'), { once: true });
    }
    ...
    el.addEventListener('mousedown', (e) => {
        e.preventDefault();
        e.stopPropagation();
        el._suppressRepositionUntil = Date.now() + 900;
